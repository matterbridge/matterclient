...
func (m *MMClient) doLogin(firstConnection bool, b *backoff.Backoff) error {
	...
	for {
		...
		if m.Credentials.Token != "" {
			_, err = m.doLoginToken()
			if err != nil {
				return err
			}
		} else {
			m.User, _, err = m.Client.Login(m.Credentials.Login, m.Credentials.Pass)
		}
		if err != nil {
			d := b.Duration()
...
func (m *MMClient) doLoginToken() (*model.Response, error) {
	...
	var err error
	m.User, resp, err = m.Client.GetMe("")
	if err != nil {
		return resp, err
	}
	if m.User == nil {
...
func (m *MMClient) wsConnect() {
	...
	wsurl := wsScheme + m.Credentials.Server + model.APIURLSuffixV4 + "/websocket"
	header := http.Header{}
